...
const manageVisitAgain = useCallback(() => {
  if (typeof window === "undefined") return; // Safety for SSR

  const existing = localStorage.getItem("visitedStores");
  if (existing) {
    let visitedStores = JSON.parse(existing);
...
}, [storeDetails, distance, initialSet]);
useEffect(() => {
  if (storeDetails) {
    manageVisitAgain();
  }
}, [storeDetails, manageVisitAgain]);
  ...
  }, [configData, storeDetails , dispatch , manageVisitAgain]);
...
export const getServerSideProps = async (context) => {
  ...
  const { req } = context;
  const language = req.cookies.languageSetting || 'ar';
